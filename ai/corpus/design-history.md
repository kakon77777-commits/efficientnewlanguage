...
exactly what CPython would (gated by an `interp == python` test), and emits a **eml-trace-v1**
...
expansions, the two-stage `^+` rule, the eml-trace-v1 envelope + event vocabulary, the
