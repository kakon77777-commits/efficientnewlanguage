...
3. **好裝好跑** — `@eml/*` 發佈到 npm、`npx eml`、單檔 `eml` 執行檔（Node SEA）、Python 端 runtime helper。
   **`npx eml`/npm 打包半段已完成（MVP）2026-07-19。** `packages/cli/package.json` 重寫：`bin` 指向
   `dist/index.js`（編譯後產物，非原始 `.ts`）；新增根層 `esbuild` devDependency + `build`/`build:cli`
   腳本，把全部 10 個 `@eml/*` workspace 依賴打包進單一輸出檔（純內部 TS 原始碼，無自己的發佈需求，
   適合內嵌），唯一真正的外部 npm 依賴 `@anthropic-ai/sdk` 保持 external（`--external:` + 留在
   published `dependencies`）；補齊 `files`/`license`/`author`/`repository`/`engines` 中繼資料。過程中
   透過真實的「打包後在 repo 外部全新資料夾 `npm install` 該 tarball」驗證（而非只跑內部測試）抓到一個
   真正會斷的 bug：`@eml/symbols` 原本用 `import.meta.url` 算固定 `../../../` 相對路徑找
   `eml-symbols.json`，一旦被 esbuild 打包成單檔，bundle 自己在磁碟上的位置不再符合原始原始碼樹狀結構，
   同樣的相對位移解出完全錯誤的路徑（外部安裝會 ENOENT）——修正為「從模組自身位置往上逐層尋找
   `eml-symbols.json`」的走訪函式，並讓 `build` 腳本把 `eml-symbols.json` 複製到 `dist/` 旁邊。修正後
   重新打包、重新在乾淨外部資料夾安裝驗證，`eml run`/`eml explain` 皆正常運作，零 ENOENT。
   835 測試全數通過（零回歸）。**Node SEA 單檔執行檔、Python 端 runtime helper、以及 `npm publish`
   本身（含套件命名決策 `@eml/cli` vs 無 scope 名稱）明確留待未來輪次**，本輪只證明「已可安裝可執行」，
   尚未真正發佈到 registry。
