...
3. **字串格式化（`%` 格式化 + `.format()`）** — 待做，兩套獨立機制。
4. **三引號字串 `"""..."""`** — 待做（常見於當 docstring 用途；`Leap_Year_Checker` 目前卡在這）。
...
8. **`not` 布林一元反轉運算子** — 待做，2026-07-17 量測 `%` 進展時意外發現（`Calculate_age` 第 21
   行 `(not leap_year)`）。規模應該很小，跟 `and`/`or` 同一套機制（短路、對任意型別做真值反轉），
   之前一直被 `%` 擋在更前面所以沒被量測到過。
