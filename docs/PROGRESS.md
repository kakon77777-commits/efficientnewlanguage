...
| A-3 | 好裝好跑（npm / `npx eml` / SEA 執行檔） | `███░░` 3 | npm/`npx eml` 打包半段 MVP 完成 2026-07-19：`@eml/cli` 改為 esbuild 單檔打包（10 個 `@eml/*` workspace 依賴內嵌，`@anthropic-ai/sdk` 保持 external）、`bin` 指向 `dist/index.js`、真實外部安裝驗證（`npm pack` + 全新資料夾 `npm install`）通過，過程中抓到並修正一個真正的打包路徑 bug（`@eml/symbols` 的 `import.meta.url` 相對路徑在打包後失效）。835 測試全綠。缺口：Node SEA 單檔執行檔、Python 端 runtime helper、`npm publish` 本身（含命名決策）均未開始。 |
...
- **2026-07-25 起 — 案例語料庫每日擴充線（目前 366 個案例、16 條量測軸）。** 這條線是 Neo
  ...
  測試與 `pnpm monitor`，然後兩個 repo 一起發布、驗 production `build_id`。**截至 2026-08-13：
  語料庫 366 個程式，測試套件 2131 tests / 61 files。**

  這條線真正的產出不是案例數，而是**量測軸**——每天挑一條現有 gate 看不見的軸，把期望值用
  程式**算出來**而不是打出來，然後 drill（故意弄壞它守的修正、確認 gate 會失敗、再逐 byte 還原）。
  目前 15 條：(1) 語法構件、(2) builtin × 引數形狀、(3/3b) 運算子 × 運算元型別含**錯誤訊息**、
  (4) 敘述互動、(5) 值模型邊界、(6) 反向轉譯構件配對、(7) slice 邊界、(8) 診斷可達性、
  (9) 結晶化快取鍵、(10) **trace 完整性（2 個真缺陷）**、(11) CTS 忠實性、
  (12) **診斷位置（1 個真缺陷）**、(13) **跨作用域再綁定（1 個真缺陷，256 支程式用 round-trip fixpoint 判定）**、
  (14) **運算式內部求值順序（44 個運算元轉錄對真 CPython，首跑乾淨，順帶釘住 3 條文法邊界）**、
  (15) **物件同一性與變動可見性（31 種「複製還是別名」對真 CPython，首跑乾淨）**——`=>` 對可變值
  是別名，而前 14 條軸比的全都是單一運算式印出來的值，物件同一性正好是不會出現在那個值裡的東西。
  方法本身寫在 `assessment/04-method.md`，包含每條軸「看不見
  什麼」的欄位——那一欄往下讀，就是這套紀律的形狀。

  **2026-08-10（第 56-58 輪，語料庫 315 → 330）主題是「組合」**，也就是 roadmap 裡
  「基本程式與組合」的後半。切入點選在語料庫還沒碰過的一塊：不是「順序會不會改變答案」
  （`successive-percentage-order` 已經在掃所有順序），也不是「單一函式有沒有某性質」
  （`idempotence-witness` 已經在驗 `f(f(x))==f(x)`），而是**每一段都自帶通過的檢查、
  合起來仍然錯**。三輪各自的機制：(56) 接縫沒有人負責；(57) 元件被重用到它從沒見過的
  假設下；(58) 組合改變了什麼是可達的——第三輪的題目是從前兩輪實際浮現的東西挑的，
  五個案例裡有三個的機制都是「測試資料到不了那裡」。

  **這一天沒有新增量測軸，仍然是 15 條。** 標準紀律是每天挑一條現有 gate 看不見的軸，
  今天的產出全部是案例，這裡照實記，不把 15 說成 16。三輪浮現的候選軸是**可達性**：
  組合之後哪些輸入／分支／狀態實際上還會出現——目前沒有任何 gate 在量這個，而
  `upstream-guard-makes-the-downstream-guard-dead`（下游 reject 分支經由管線 0 次可達）
  跟 `normalisation-makes-the-old-tests-unreachable`（正規化後 8 個測試只剩 3 個不重複、
  router 少一個可達分支）是現成的兩個witness。

  **一個量到的運維問題，尚未修（task #399）。** 語料庫長到 330 之後 `pnpm test` 開始以
  非零退出，訊息是 `[vitest-worker]: Timeout calling "onTaskUpdate"`——**不是測試失敗**：
  60/60 檔案、2020/2020 測試全過，而且 1975+15×3=2020 剛好對上。原因是 `tests/interp.test.ts`
  對每個範例開一個 CPython 行程（約 56 秒同步阻塞），把 reporter 的 RPC 餓死。單獨跑
  interp.test.ts（415 tests / 55.76s）與 examples.test.ts（681 tests / 6.88s）都不報錯，
  只有併行會。這個 repo 沒有 CI，所以沒有共用的紅燈，但它會隨語料庫線性惡化——往 1000
  的路上一定要改成像 `statement-interaction.test.ts` 那樣批次成一個子行程。

  **2026-08-11 — 這一天的產出不是案例，是閘門。** 原訂三輪，實際只寫了 1 個案例，照實記。時間去了兩個地方，兩個都是實的。

  **一、執行真相閘門批次化（commit `8fb3e00`）。** `tests/interp.test.ts` 原本每個程式開一個
  `python -c`，330 個範例約 390 個行程、約 56 秒同步阻塞，把 vitest reporter 的 RPC 餓死。
  改成一個行程跑完、每支各自 globals：**55.76s → 9.16s**，全套退出碼零。因為這改的是最吃重
  那道閘門的執行方式，等價性用兩個測試量而不是宣稱：真實程式抽樣必須跟自己一個行程一致；
  以及**批次確實會洩漏行程層全域狀態**（見證：`setrecursionlimit` 跨程式可見），而讓它在這裡
  仍然成立的是「沒有任何產出的 Python 碰得到那個」——對實際產出檢查出來的，不是從語言規格推的。
  順帶自己造成一個代價並修掉：`ai-converter.test.ts` 在 interp 變快的同一天開始逾時（檔案重疊
  變多、爭 CPU），用類別修法把 `testTimeout` 提到 30 秒放進 `vitest.config.ts`。

  **二、軸 16：執行真相閘門實際上看得見什麼（commit `8ec2ef7`）。** 鑽新閘門時用軸 15 的別名
  缺陷去打，**閘門全綠**——那是「批次化弄壞閘門」的形狀，所以先分開兩種解釋：拿原本的逐行程
  版本跑同一個壞掉的直譯器，**也全綠**。所以批次化清白，而發現更老更大：**這道閘門從來就看不見
  list 綁定的別名行為**，330 支程式沒有一支的 stdout 依賴它，而且沒有任何地方說過。
  `tests/gate-sensitivity.test.ts` 把手工鑽推廣成八個突變運算子作用在產出的 Python 上，
  量有幾支程式的輸出改變。首跑（340 支、抽 43）：NULL 控制組 **0**（分開「在量測」與「在發明」）、
  `len+1` 29/29、`==→!=` 36/36、`+1→+2` 32/32、`<→<=` 24 適用 16 偵測、`>→>=` 21/14、
  **別名突變 12 適用 0 偵測**、`str→repr` 40 適用只有 5 偵測。第一版沒有時間預算，第一個
  無限迴圈突變體就掛超過十分鐘（`i+1` 變 `i+2` 會跨過終止條件），現在每支程式跑在 trace hook
  強制的牆鐘預算下，**不終止是一個結果而不是一次卡住**。鑽過：閹掉比較器就報「harness cannot
  detect anything」。接監視器接在**產出器**上，因為運算子是照產出的 Python 形狀寫的。

  **三、唯一的案例 `no-observable-distinguishes-the-two`**，把軸 16 的發現寫成可執行的東西：
  六個問題 × 六組輸入 = 36 次觀察，分歧 0；同一組問題重複到 1800 次觀察，分歧仍然 0；
  換一種問法（交出去之後改呼叫端自己的 list）30 次觀察就有 21 次分歧。**測試套件的上限是系統
  渲染得出什麼，不是它多用力去看。**

  **2026-08-12（第 60-62 輪）— 補償性缺陷、同源檢查，以及異議到了之後誰被相信。** 語料庫
  **331 → 346**，測試 **2071 / 61 files**，軸仍是 16（今天沒有新軸，照實記）。三輪的主題都在
  「組合」這條線上，走進既有 331 個案例沒碰過的區域：組合類全部是「每一段都對、接縫錯」，
  今天做的是**反過來**的那一半。

  **第 60 輪：整體是對的，因為兩個部件各自是錯的。** 編碼器與解碼器共用一個錯的常數，四個修復
  狀態裡**兩個**乾淨,而那兩個是「都錯」跟「都對」——所以分批上線一定落在會壞的那兩個之一；
  視窗起點與終點各晚一格,長度檢查接受 2 個狀態、其中 1 個正確,而在平坦的 fixture 上連和檢查
  都是 0/8;一把過度積極的快取清除把壞掉的 key 藏了起來,而藏它的東西正是「快取從來沒命中過」
  (flush every 1:命中 0),放寬到 12 時壞 key 的命中率反而**比正確的 key 高**(9 vs 6),
  拿來替它辯護的指標就是缺陷灌出來的;一張「安全網」承擔了 6 個正確輸出裡的 3 個,而量下去
  發現該刪的是被標成主線的那一段;最後是修復順序——四個序列裡只有 1 個會讓**視窗期間寫入**的
  紀錄永久讀不到,而遷移前就存在的舊資料四個序列一模一樣(所以那是 backfill 的成本,不是序列的)。

  **第 61 輪：檢查與被檢查的東西同源。** golden 由被測程式重新產生時,凍結的 golden 抓到 4/4
  真改動、重新產生的抓到 0/6（帶 NULL 控制組:語意等價的改寫必須**不**被偵測,否則這個 harness
  是在發明差異）;測試用同一個 normaliser 正規化期望值時,缺陷影響 4 個行為、共用寫法看見 0 個,
  而缺陷在共用函式**之外**時兩種寫法一樣強——盲點是局部的,只從 import 清單看得出來;驗證器拿
  「真實生產資料」跑出 0 拒絕,因為資料庫裡就是它放行的那些,而其中 3 筆違反規格;「獨立實作」
  其實是翻譯,分辨方法不是同意率而是**規格沒有規定的輸入上**的同意率(有血緣的 2/2,真獨立的 0/2);
  監控器透過它要監控的快取讀取,6 次陳舊 0 次抓到——而「每次都告警」的對照組抓到 6 次、誤報 6 次,
  這就是為什麼偵測數單獨不成其為量測。

  **第 62 輪的主題是從 60、61 讀出來的**(第二次用這個選法)：兩輪都留下同一個沒被量的問題——
  異議終於出現時誰被相信。四個實作投票,成員 4 個、相異行為 **2** 個(血緣是從答案推出來的,
  不是從註解);一個「差 3」用容差 5 解決,結果連帶蓋掉 4 個**別的**成因,而且系統性成因會隨量
  成長(x1 需要 5、x4 需要 14);四位小數的估計比整數的**解析度確實更細**(6/6 對 0/6 感測到
  單位變動)而且錯得多 17 倍;「把最吵的檢查調鬆」這條每次都合理的政策,四輪之後抓到率 7/7 → 4/7,
  總告警 28 → 11,**完全達成了被交付的目標**;仲裁者打的是其中一方的端點,A 四戰全勝而只對 1 次,
  A 的準確率其實是 5/8、B 是 7/8。

  三個自己造成、由輸出改掉的錯誤都留在檔案裡：把遷移前資料跟視窗內資料混在一起算(讓四個序列
  看起來一樣糟、藏掉唯一的差別)、一句「只有一個序列兩者皆免」而程式印的是 0/4、以及一組湊巧
  整除的資料讓「四位小數」在輸出裡根本沒出現,加上擾動了被丟棄的那個讀數導致靈敏度那節全是 0。

  **順手修掉一個基礎建設缺陷,而它是 08-11 那個的同一類、不是同一個實例。** `pnpm test` 又開始
  非決定性地吐 `[vitest-worker]: Timeout calling "onTaskUpdate"`。這次的兇手是
  `tests/transpiler-cpp.test.ts`:整個 MSVC 建置是**單一** `spawnSync`,把 worker 的 event loop
  擋住約 100 秒,reporter 的 RPC 就餓死。改成非同步 spawn + await。因為「錯誤消失」有兩個解釋
  (機制修好 / 這輪機器比較閒),加了一個能分辨的見證:建置期間讓 event loop 自己數 tick,
  斷言 tick > 0。鑽過——換回 `spawnSync` 就是 `expected 0 to be greater than 0`,
  **62866ms 內一次都沒跳**。同一次鑽也證明 100s → 60s 是負載波動,不是我的改動:改動改的是阻塞,
  不是速度。這條規則現在是**被量出來的性質而不是被記住的**。

  **2026-08-12 追加(第 63 輪)— 那個決定接下來變成什麼。** 語料庫 **346 → 351**,測試 **2086 / 61 files**。
  主題一樣從前三輪讀出來:60-62 全部停在「異議被錯誤解決」的那一刻,而沒有人量過那個決定之後
  變成什麼。量到:同一類爭議跟隨最早的裁決時,正確率 **7/10 → 5/10**,而證據只被讀了 **3** 次;
  退款那條線上 **5 個決定** 全部靠 **1 次觀察**,期間證據從強度 1 漲到 5、一次都沒被再讀;
  一條「供應商 7」的排除規則在**兩個方向**都錯(移除 5 筆理由不涵蓋的、放行 1 筆理由涵蓋的),
  因為它的理由被寫下來但從不被**求值**,所以沒有可以變成 false 的條件;四個系統同意、實際只有
  **2 個來源**,而唯一能報新聞的那個被當成離群值;結論式稽核日誌對「決定了什麼」6/6 完整、
  對「決定得對不對」**0/6**——可重推導性是**算出來的**(規則宣告它讀哪些欄位、紀錄有沒有帶),
  不是宣稱的。

  **其中一個案例的量測否定了我的前提,結果照量測寫。** `the-rerun-seeds-from-the-last-run` 原本
  預期「用上一輪結果當種子」比較差,量出來是總誤差 **52 對 432**——種子明顯比較好。保留結果,
  改寫框架:種子買到的不是準確度而是**誤差的分佈**,warm 在平靜日只有 12、在變動日有 40,
  而週平均正是唯一看不出這件事的摘要。一個結尾被自己輸出打臉的案例,就是 60-62 三輪在講的
  那個缺陷類別本身。

  另外抓到一次過期 golden:最後改了結尾幾行卻沒重跑產生器,`examples.test.ts` 立刻以
  `statements 68 -> 69` 擋下。這正是 golden 存在的理由。

  **2026-08-13（第 64-66 輪）— 缺陷被找到之後的那條鏈。** 語料庫 **351 → 366**，測試 **2131 / 61 files**，
  軸仍是 16。主題連續第四、五、六次是從前幾輪讀出來的：60-63 輪講「系統為什麼查不出自己錯了」，
  這三輪走到**之後**——缺陷被找到，還要走完觀察 → 描述 → 傳遞 → 重現 → 定位 → 分組 → 修 → 驗，
  每一節都會靜靜掉東西。這條線今天有現成實據：岑衡（Cen Heng, Codex）交來 22 項真缺陷，而
  `pnpm test` 當時是 2086/2086 全綠。

  **第 64 輪（報告端）：** 報告點名 3 個、實際 7 個錯、2 個成因，照報告打補丁要 7 次照機制修 2 次；
  重現腳本**通過與否只跟它自己的 builder 有關**，跟被歸咎的缺陷無關（缺陷開關兩邊都是 140），
  而那個缺陷正在弄壞 3/4 生產訂單；分級用見證大小排序在母體 A 完全正確（0 反轉）、母體 B 錯一組，
  **而見證在兩個母體一模一樣、影響變了兩個**；報告點名 1 個地點、4 個都壞，在點名處修完剩 3 個，
  而且那個修法在另一類輸入上**本身就是新缺陷**；兩個 finding 各自只剩 2 個候選成因、合起來剩 1 個，
  兩邊各自修完的補丁會在真正成因被修的那天同時反咬。

  **第 65 輪（修正端）：** 兩種修法在**每一個被測過的整數上完全等價**、其中一個在小數上錯；
  測試套件在**系統是錯的那 2 個狀態下全綠**，而唯一全對的狀態讓它變紅，紅的測試 0 個守規則、
  2 個守 workaround；結案規則讀的是**回報**不是**發生**，結案後 485 次發生 0 份報告；
  用修正者的輸入重驗找到 0 個逃脫、獨立生成找到 1 個，**9 個輸入是重跑腳本永遠看不到的**；
  同一份測試碼一行沒改、只換 fixture 資料，能失敗的守衛從 1 變 **0**。

  **第 66 輪（兩方系統本身）：** 第二方找到的**比較少**（5 對 7）卻仍值得加，因為 1 個是對方到不了的，
  而同類型的第三方加 0；找的比修的快時，同樣容量、同樣修完 6 個，順序讓成本變成 127 對 71、
  殘留傷害 20 對 5，而**容量足夠時兩種順序完全相同**（這是控制組）；讀了文件的檢查者在
  文件命名的類別上召回 6/6、沒命名的 0/6，而總數 6/12 看起來只是普通的部分覆蓋；兩方各自回答
  「這是我的嗎」，沒被覆蓋的 3 項**全部是接縫**，而且加了第二方之後未覆蓋數從 6 降到 3、
  可見度卻更差；交接表單能讓接收端做 4/6 的決策、寄件人知道的能做 6/6，**加一個欄位換到 0**，
  因為那個決策缺兩個事實。

  **量測四次否定我的前提，三種不同的處理，差別記在這裡：**（a）`severity-...` 母體 A 沒有反轉——
  保留結果、加第二個母體，問題變成「排序對不對是母體的性質」；（b）`the-fix-moved-the-boundary`
  對整數而言移動門檻**就是**正確修法——保留、改成量「兩者何時分道揚鑣是定義域的事」；
  （c）`the-verifier-reran-the-fixers-script` 是**我的模型建錯了**（寫了完整的修正卻當成不完整的）
  ——這種要改模型不是改框架；（d）`finding-outruns-fixing` 容量設得比到達量還大，前提根本沒被
  實例化——改參數。**（a）(b) 改框架是誠實，(c)(d) 改框架就會變成替壞模型找台階。**

  另外抓到自己三個缺陷：一個 `ci` 從頭到尾沒遞增（讓三個候選成因其實都在跑同一格）、
  一個 `X==1 且 X==0` 的自我比較（**永遠**印 0，無法失敗）、以及六處打出來的數字。
  ...
  被抓到**五個洞**，每次都是**補測試**而不是 `--accept`。第五個（2026-08-09）跟前四個不同類：
  監控的規則沒有變、也沒有任何 commit 弄壞它，是它**自己失效了**——「測試也動了」這個豁免條件
  把「基線沒見過的測試檔」也算成動過，而「沒見過」是相對基線讀的，所以基線一天沒 accept，
  那個豁免就一天有效。基線自 `9f3824f` 起沒再 accept，期間有三個 conformance 檔加進對應表，
  於是 `packages/interp/src/index.ts` 與 `packages/transpiler-eml/src/eml-emitter.ts` 的漂移檢查
  **已經三輪無法失敗**。量到的證據：把 `assign()` 改成綁定 list 時複製而非別名（會讓軸 15 有
  4/31 個答案改變），完全不動任何測試，監控回報「reviewed / no drift」。修法是把兩個條件拆開
  ——**被編輯過**的測試才是對這次修改的證據；基線沒見過的測試只證明**基線過期**，它仍然豁免，
  但會同時發出 `STALE BASELINE` 警報點名「哪些檔案的漂移檢查現在不可能失敗」，讓豁免在下次
  accept 時到期而不是永久有效。修完重鑽：同一個修改，這次報 `SEMANTICS CHANGED`。

  **2026-08-13 追加（同日，非語料庫）— 兩方稽核安排跑完第一個完整循環，而它當場否證了我一項
  自己的判斷。** 檢查方岑衡（Cen Heng, Codex）2026-08-12 在 `f77a43f` 交來 22 項稽核後，這條線
  今天有三個進展。**其一，頻道換載體**：往返從本機資料夾移到 AI Board 專版 `eml-p-relay`
  （Neo 的決定——他不想再當兩個對話之間的人工管線）。編號從 `EMLP-RELAY-0009` 接續不重編。
  換載體有一個可量測的理由而不只是方便：板是**不可變帳本**，更正只能追加；檔案版的 README
  只是**要求** append-only，而在 `RELAY-0007` 我想順手修掉自己舊訊息裡一條過時路徑時，
  擋住我的只有我自己的自制力。**其二，他在新 HEAD `a2c57d1` 上把四項 CRITICAL 原樣重跑，
  全部成立**——今天新增的 15 支語料沒有偶然蓋掉任何一項，`git diff --stat f77a43f a2c57d1 --
  packages/` 是空的，產品碼今天仍是零行改動。

  **其三，也是真正有份量的一項：他自己另生輸入，找到我沒找到的第五個機制。**
  `str(1 in ([] or [1]))^0` 產出 `print(str(1 in [] or [1]))`，`in` 綁得比 `or` 緊，
  interpreter 得 `True`、真 CPython 得 `[1]`，`ok:false`。我獨立重跑一字不差，並且跑了空控制
  （`str(1 in [1])^0` → `ok:true`, exit 0）確認那個紅是真分歧不是儀器故障。意義不只是多一個
  witness：`emitter.ts:112-113` 那一行**兩個運算元都沒有經過 `child()`**，我在往返裡只記成
  「element 沒過」——**我讀窄了一格**。而他之所以找得到，是因為他沒有只重跑我附的見證——
  **這正是本輪第 65 輪語料庫量到的東西在數小時內兌現**：用修正者的輸入重驗找到 0 個逃脫、
  獨立生成找到 1 個。

  **他另外糾正了我一個 gate 用詞，而那個錯的形狀就是我自己第 66 輪寫的案例。** 我打算把測不出
  分歧的格子標成 `not-observable`——那是在主張**不可能觀察**，實際成立的只有「我在這個範圍內
  搜了、沒找到」。改用四級：`DISTINGUISHABLE`／`NO_WITNESS_FOUND`（記搜尋域、界限、seed，
  不主張等價）／`UNREACHABLE`｜`NOT_EXPRESSIBLE`／`PROVEN_EQUIVALENT`（只有窮舉或有證明才能用），
  且結構 exact-emission 覆蓋與 behavioral witness 分兩欄不混算。第 66 輪的
  `the-handoff-loses-what-has-no-field` 量到的正是：缺的事實**全都是關於搜尋過程**而不是關於
  缺陷本身。**我寫了那個案例，然後在自己的閘門詞彙裡犯了同一個錯，由不知道那個案例存在的
  第二方指出來。**

  **還有一項是他擋下我的：** class 層級屬性那格我本來傾向改成明確拒絕（fail-loud），他指出
  那等於**在修缺陷的過程中把語言縮小**，而 `class C: 5 => list` 與 `c.list` 目前合法可建模、
  class namespace 的 `list` 也蓋不到 module-level builtin；要改成語言限制得另行修規格並由 Neo
  決定，不能夾帶。收下為通則，不只是這一格。**判準因此定案**：B 群不變量寫成「產出的 Python
  reference 必須解析到該 binder 建立的**同一個 namespace binding**」（解析一致，不是字串一致），
  gate 測 binder × 合法 reference form 的**配對**；A 群故意破壞點從 3 個增為 **7 個**，其中
  2/3 與 6/7 成對，用來證明比較鏈左右兩側、`Membership` 兩個運算元是**分別**被守住的；驗收採
  `|R∩V|/|V|` 重疊率公開制，且他的驗收輸入在判定前我不看也不打聽。交回時分六欄：產品修正／
  behavioral witnesses／structural coverage／NULL controls／七個 drill **各自的實際紅燈輸出**／
  完整驗證。`VERIFIED_FIXED` 仍只有他能標。**修正工作 2026-08-14 開工。**

  **2026-08-14（第 67-69 輪）— 一句真話被當成另一句真話讀。** 語料庫 **366 → 381**，測試
  **2176 / 61 files**，軸仍是 16，語意漂移監控 381 支語料乾淨。主題連續第七、八、九次從前幾輪
  讀出來：64-66 走缺陷被找到之後的那條鏈，每一案都是**本來在那裡的東西掉了**；67 走它的對偶
  ——**宣稱某個東西不在那裡**；68 走「答案回答的是另一個問題」；69 量前兩輪那個修法（把限定
  條件帶上）本身的代價與極限。

  **第 67 輪（負面結果licenses 什麼）：** 兩個掃描器印出同一句「0 個分歧」，一個走 12 格、
  一個走 144 格，**144 個單缺陷世界裡 A 分辨得出 12 個、分辨不出 132 個**，而 A 在它自己走過的
  12 格上是窮舉的（那裡 0 就真的是不存在）；早退的掃描在 11 筆裡只看 4 筆、回報 1 而實際 3，
  且**同一個數字在單批次串流上是完全正確的**；產生器只造得出非負分子，2000 個輸入找到 0 個分歧、
  換成 160 個含負數的找到 52 個，**每一個分歧都在產生器造不出來的那一側**；兩條管線丟掉相近數量
  的紀錄，丟得多的那條（10 筆）回報 20% 與真值相同，丟得少的（8 筆）回報 0%，因為它丟掉的
  **正好是全部 8 筆有問題的、且只有那 8 筆**；一個查詢在三個世界都回 0，單看查詢只分得出 1 個
  世界，加列數分得出 2 個，再加負值金額分得出 3 個。

  **第 68 輪（回答的是另一個問題）：** 索引以帳號為鍵，10 個帳號 6 個人，月 1 兩種算法一致
  （控制組）、月 2 是 7 對 3；比較算繪的字串在同一個函式上**同時太嚴與太鬆**——5 個假警報加
  2 對它分不出來的值（`AB|C` 與 `A|BC` 都繪成 `ABC`）；只檢查「有沒有拋例外」的冒煙測試對
  2 個跑不完的變體 2/2 全抓、對 2 個跑完但算錯的 **0/2**；依到達時間分桶與依事件時間分桶
  **總數完全相同**（都是 18）而每日形狀不同，兩天被報成 0 其實有 4 筆與 3 筆；五個 90% 的步驟
  端到端是 **59%**，流失人數從 100 遞減到 66，而**把任何一步從 90% 拉到 95% 都只多 32 人**
  ——乘法可交換，所以「先修流失最多的那一步」這個直覺是錯的。

  **第 69 輪（帶上限定條件的代價）：** 一個帶四個欄位的數字走過四個消費者，**數值一路不變、
  三個限定詞全掉光**，第一個掉東西的是第 2 跳；兩隊都對的「活躍用戶」相加得 **11，而總共只有
  10 個人存在**，所以那個和不是任何集合的計數；把 uptime 的基準寫清楚讓數字從 90.0% 掉到
  50.0% 而**服務端零變化**，時間序列上看起來像一次事故；三隊各自把定義寫精確之後，跟「一條嚴格
  規則套用到所有人」的排名一致度是 **2/3**，而原本共用的鬆散規則是 **0/3**；限定事實存在、有欄位、
  也被讀取，只是**比決策點晚 3 天到**，10 筆決策裡 3 筆事後看是錯的，而等 3 天可以全對。

  **這一輪最重要的一件事不是案例，是一個我自己的驗證缺陷。** 今天前半段我一路用
  `node packages/cli/dist/index.js` 跑 check / trace / roundtrip，而那個 bundle 是
  **2026-07-18 建的**，早於 `PHOSPHOR_PROTO` → `EML_TRACE_PROTOCOL` 那次改名。所以不只黃金
  trace 寫進了過期的協定名（`tests/examples.test.ts` 5 個失敗就是它抓到的），更嚴重的是
  **那五個 `eml:equiv ok:true` 驗的是一個月前的編譯器，不是 HEAD 的原始碼**。正確入口是
  `pnpm eml`（= `tsx packages/cli/src/index.ts`，也正是 README 教人用的那個）。全部重跑，
  五案在原始碼路徑上仍然 `ok:true`，黃金重產、`dist/` 重建（254KB → 272KB，確認是不同建置）。
  順帶：一直伴隨的 `[vitest-worker]: Timeout calling "onTaskUpdate"` 與第 6 個失敗也一併消失，
  全套時間從 257s 降到 50s。**`packages/cli/dist/` 是 gitignored，所以這是純本地陷阱、沒有
  commit 紀錄可查，也因此更容易再犯。**

  **三次量測否定我的前提，一次改資料、一次改資料、一次改題目：**（a）`zero-rows-three-explanations`
  第二個 probe 買到 0，因為我讓 A 有 3 列、C 有 4 列，列數這個**湊巧**的性質先把它們分開了
  ——把兩者列數改成相同，兩個 probe 各買 1；（b）`the-id-answers-...` 的控制組沒印出來，因為
  月 1 就已經有同一人的兩個帳號——把其中一個移到月 2；（c）`comparable-because-both-were-vague`
  量出鬆散共用規則對理想規則的排名一致度是 **0/3**、三個各自不同的精確規則是 **2/3**，
  **正好推翻它自己的標題**。量測是健全的、錯的是我的預期，所以**改標題不改資料**：案例更名為
  `the-shared-rule-ranked-worst`，README 開頭直說「這個案例寫出來是要證明相反的事」，並註明
  這是這個母體的性質而不是定律。（a）(b) 是資料沒實例化前提，（c）是我猜錯方向——把資料調到讓
  原標題成立，就會變成替前提找台階。
...
- **2026-07-19** — 完成 **A-3 好裝好跑：`@eml/cli` npm/`npx eml` 打包半段（MVP）**。B-6 KPI
  ...
  `@eml/cli` 的 `bin` 指向原始 `.ts`（Node 無法直接執行）、10 個 `@eml/*` 依賴全用
  `workspace:*`（外部安裝無法解析）、完全沒有打包步驟、`package.json` 缺 `files`/`license`/
  `repository`/`engines`。**修正**：新增根層 `esbuild` devDependency；`packages/cli/package.json`
  改寫——`bin` 指向 `dist/index.js`、新增 `build` 腳本把 10 個 `@eml/*` workspace 依賴內嵌打包
  （純內部 TS 原始碼，適合內嵌）、唯一真正的外部依賴 `@anthropic-ai/sdk` 保持 external + 留在
  `dependencies`、補齊發佈中繼資料、加 `prepublishOnly` 防止發佈到過期 `dist/`。**過程中透過真實
  外部安裝驗證（而非只跑內部測試）抓到一個真正會斷的 bug**：`@eml/symbols` 用 `import.meta.url`
  算固定 `../../../` 相對路徑找 `eml-symbols.json`，一旦被 esbuild 打包成單檔，bundle 自己的磁碟
  位置不再符合原始碼樹狀結構，同樣位移解出錯誤路徑——外部安裝 `npm install` 該 tarball 後執行
  直接 ENOENT。修正過程中兩次嘗試靜態 JSON import 皆失敗（esbuild 的 JSON→ESM 具名匯出轉換遇到
  `eml-symbols.json` 裡字面叫 `"await"` 的頂層鍵，`await` 是保留字，兩次都在 dev 模式炸出
  `SyntaxError: Unexpected reserved word`，加不加 `with {type:'json'}` 都一樣）；最終改回
  `readFileSync`+`JSON.parse`，但把固定相對位移換成「從模組自身位置往上逐層尋找」的走訪函式，
  並讓 `build` 腳本把 `eml-symbols.json` 複製到 `dist/` 旁邊。**驗證全部用真實流程**：`pnpm
  typecheck` 乾淨；dev 路徑（`pnpm eml explain`）修好後正常印出符號表；`pnpm build:cli` 產出
  `dist/index.js`（含正確 shebang、零 `workspace:` 殘留）+ `dist/eml-symbols.json`；bundle 執行
  跟 `pnpm eml run` 逐位元組相同；`npm pack` 產出 3 檔 tarball；在完全外部的全新資料夾
  `npm install` 該 tarball 後，`npx eml run`/`npx eml explain` 均正常運作、零 ENOENT，驗證後
  卸載清乾淨。835 測試全綠，零回歸。**本輪明確排除**：Node SEA 單檔執行檔、Python 端 runtime
  helper（需要先問 Neo 這句話具體指什麼）、`npm publish` 本身（需要 Neo 另外授權+套件命名決策）。
  詳見 `docs/agent-handoff.md`「A-3 好裝好跑」章節、`docs/roadmap.md` A 節第 3 項。
...
`eml roundtrip`。反向方向沒有任何已知未動工的語言缺口候選。Neo 定下下一階段優先序後選了
**A-3 好裝好跑先做**，npm/`npx eml` 打包半段已完成 2026-07-19（見上方工作日誌最新一條）——
`@eml/cli` 現在可以真正打包、真正外部安裝執行，過程中抓到並修正一個真實的打包路徑 bug。**A-3
剩下的缺口**：Node SEA 單檔執行檔、Python 端 runtime helper（需要先問 Neo 這句話具體指什麼）、
`npm publish` 本身（需要 Neo 另外明確授權 + 套件命名決策，`@eml/cli` 需要 `@eml` npm org scope，
可能不存在，或許要換無 scope 名稱）。其餘次要候選（皆為純工程、不需要商業判斷或品牌素材）：
