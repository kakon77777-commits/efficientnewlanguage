...
- **2026-07-25 起 — 案例語料庫每日擴充線（目前 846 個案例、16 條量測軸）。** 這條線是 Neo
  ...
  測試與 `pnpm monitor`，然後兩個 repo 一起發布、驗 production `build_id`。**截至 2026-09-14：
  語料庫 846 個程式，測試套件 3733 tests / 72 files（前一日 monitor drift-guard stack 落地新增 2 個測試檔）。**
