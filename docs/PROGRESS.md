...
- **2026-07-25 起 — 案例語料庫每日擴充線（目前 861 個案例、16 條量測軸）。** 這條線是 Neo
  ...
  測試與 `pnpm monitor`，然後兩個 repo 一起發布、驗 production `build_id`。**截至 2026-09-15：
  語料庫 861 個程式，測試套件 3778 tests / 72 files（語料日，測試檔數不變 72；monitor 861 no-drift、exit 0；3778-3733=45=15×3）。**
  ...
  **2026-08-15（第 70-72 輪）— 讀者不是被動的。** 語料庫 **381 → 396**，測試 **2221 / 61 files**，
  軸仍是 16，語意漂移監控 396 支乾淨。主題連續第十、十一、十二次從前幾輪讀出來：67-69 的讀者都是
  **被動**的——收到一個數字然後讀錯；70 走「讀者採取行動，而行動改變了被量的東西」；71 走那五個
  失敗的共同修法（留一塊不動的對照）本身；72 走「數字支持不了、但決定還是得做」的那一步。

  **第 70 輪（行動改變了量測）：** 代理指標在成為考核目標前，對目標的排序一致度是 **100%**，
  之後**連一對可比較的都不剩**（每一封回覆都同樣快），而代理進步 4.3 小時、目標退 6 件；
  只有 A 段有儀表板，把工作挪去 B 段讓圖表顯示 **66% 改善**而系統總成本**變差 9**；容量預測
  900、實際 570，**那 330 的差距 100% 是照著預測去設上限造成的**，隔年照「高估」下修反而對真實
  需求低估 25%；相對門檻（審最差的 25%）四輪下來**每輪都是 3 件**而總缺陷 60 → 25，絕對門檻
  第二輪就清空但停在 41——**A 系統更好卻報不出來，B 報得出來卻走得更短**；點擊排序在第 3 天鎖死
  在 **66%** 再也到不了 100%，而把每個位置的曝光拉平，同一個排序器**剛好回到 100%**。

  **第 71 輪（對照組本身）：** 沒有對照組時「轉換率上升 12.0」，有對照組時**同一份資料是 0.0**，
  而在真的有效果的世界裡對照組報 9.0 不是 0（這是控制組，否則會讀成「對照組只會報沒效果」）；
  把所有可能的連續對照組窮舉，離散度從大小 1 的 **8.5** 收到大小 8 的 **0.6**，而**離散度在兩個
  效果量級下完全相同**——它是單位的性質不是效果的性質，但效果 1.0 時大小 1、2 會報負值，
  「一定答得出來」的最小對照組從 5% 變 20%；控制組被排除在新排序器之外卻共用被實驗組暖起來的
  快取，**效果只剩 50%**，而數字依然正、依然穩、依然是對的形狀；全量上線只剩時間前後可比，
  而付費流量同時翻了三倍，分解後**流量結構單獨會讓數字變差 2.3 個百分點**，回報的 1.7 反而
  低估了改動本身的 4.0；最後量對照組的帳單——好世界裡花 **320**、壞世界裡省 **2200**，
  而**小到無法判定的對照組付了錢什麼都沒買，比全量上線更差也比有效的對照組更差**。

  **第 72 輪（數字支持不了，決定還是得做）：** 事後才選門檻，規則仍能否決 3/10，但事前固定的
  能否決 6/10——它是「所有可辯護門檻的聯集」；五個指標、**效果真值全為零**，「任一個動了就上線」
  在 8 次發布裡有 **7 次**有東西可報，而預先指定單一指標只有 3 次；整體效果是零時按 segment 切，
  **10 個裡有 4 個是正的**；等待每週有成本卻沒有負責人，第 **17** 週起等待比做錯更貴；
  分析被閱讀、被討論、算得也對，但跑遍 8 種可能結果只產生 **1 種決定**——它能改變的是**4 種
  對外說法**，「數字在做事，只是不是它被找來做的那件事」。

  **三次量測否定我的前提，處理方式各不相同。**（a）`the-holdout-shrank-...` 我預期小對照組會
  報零或負值，效果 6 時**從來不會**——保留該結果，**加第二個效果量級**放在旁邊，因為能不能判定
  是效果**相對於**背景變動的性質；（b）`fixing-the-flagged-ones-...` 量出相對門檻雖然報不出
  進度、卻**走得更遠**（總缺陷 25 對 41），把這個權衡補印出來而不是只講前半；
  （c）`the-analysis-ran-until-it-agreed` 「最大漲幅在最小 segment」在資料上**不成立**，
  改成無論成不成立都印出比較結果，不讓「沒印出來」變成靜默。

  **自己的三個缺陷，都是讀輸出而不是讀程式碼抓到的：** 一個找最小值的迴圈**索引從頭到尾沒遞增**
  （**跟 2026-08-13 抓到的 `ci` 同一類，我犯第二次**）；一個 `show()` 對負數算錯，`-23` 印成
  `-2.7` 而非 `-2.3`（`int()` 向零截斷、`%` 向下取整，兩者在負數上不一致）；一個**無條件印出
  「兩者不會相加、它們有交互作用」**而量測說它們剛好相加。另外一支哨符 `-1` 被當成百分比印成
  `-1%`，改成明寫 `n/a - no decidable pairs`——語料庫不該自己犯它正在記錄的錯。

  **昨天的陷阱有回頭驗。** README 是用 `packages/cli/dist/index.js` 產的，所以對其中一支做了
  dist 與 `pnpm eml`（原始碼）的**逐位元組比對**：相同（1051 bytes），並用一個不存在的檔案當
  空控制確認兩邊的錯誤輸出也一致、比對路徑是活的。

  **2026-08-16（第 73-75 輪）— 制度產物被當成它所代表的東西。** 語料庫 **396 → 411**，
  測試 **2266 / 61 files**，軸仍是 16，監控 411 支乾淨。主題連續第十三、十四、十五次從前幾輪
  讀出來：70-72 走完「量測 → 行動 → 對照 → 決定」，全都停在**這一次**；73 走**下一次**
  ——記錄下來的東西有沒有改變第二次發生的結果；74 從 73 第 1 案讀出「事故的**名字**涵蓋 5 個
  機制」，整輪量**分類本身在做沒人指派給它的工作**；75 走「彙總取代了個案」。

  **第 73 輪（記錄與下一次）：** 去年的修法還在、還有效、沒退化，但事故的名字涵蓋 **5 個機制**
  而它只關掉 **1 個**，於是「以這個速度還有 4 次事故，每一次都會讀成上一次的重複」；六個
  postmortem 行動項**全部結案**，其中 **1 個**擋得住事故的任何一步，而把同一個項目挪到第 1 步
  （結案率、數量、審查全不變）事故從第 1 步就停；一條 2021 年寫的規則在兩個時代**拒絕完全相同
  的 6 個批次、下游失敗都是 0**，一個時代每次拒絕都防住了失敗、另一個一次都沒有，而**它的紀錄
  在兩個時代讀起來一模一樣**；教訓可以存的 7 個地方只有 **3 個**會在需要的當下抵達，而「撐過
  人員流動」和「抵達」是**獨立的兩件事**（5 個撐得過、其中 3 個抵達）；MTTR 改善 **56%** 而
  使用者經歷的總停機**分毫不差還是 28 小時**。

  **第 74 輪（分類）：** 兩套分類法各自完整、各自無歧義、總數都是 12，而各取一個數字相加會
  **超過母體**；同樣三條規則的六種順序讓**每一個類別的計數都能從 1 跑到 6**，而會動的正好是
  匹配一條以上規則的那 7 項；catch-all 從 4% 長成**比任何具名類別都大**，裡面有 4 種不同的東西、
  每一種若今天重寫分類都會是具名類別；把「critical」的定義收緊（**是對的**）而沒有重述歷史，
  月 4 的 50% 下降**整段都是定義**，真正的變化在月 5、比定義造成的那一步**還小且來得更晚**；
  工單數掉 **54%** 而底下的事件數**一個都沒少**。

  **第 75 輪（彙總取代個案）：** 每個數字都對的週報，打開 **2 筆**紀錄就看見它沒有欄位可表達的
  分組；平均 5 seats／60 GB，而**在任何容忍度下都沒有任何一個客戶**同時接近兩個平均；錯誤訊息
  排行榜的前三名有三條是**同一個成因**，合起來 **57%**、比榜首那條字串的 26% 大得多；帳目總額
  **分文不差對上**而**每一行都是錯的**（淨誤差 0，因為誤差在行間移動）；清洗規則以「離平均多遠」
  定義，而被調查的問題本身就是「為什麼有些點離平均很遠」，清洗後的資料裡**該現象一個實例都不剩**。

  **今天最該記的是我自己的一個重複缺陷模式：寫在散文標題裡的數字。** 四次：「400 records」
  （資料是 12 筆）、「down 40%」（量出來 54%）、「still runs end to end」（實際停在第 3 步）、
  「total damage went up」（實際分毫不差）。全部改成由程式算出來或不寫數字。這跟 08-14 的
  「480」是同一類——**註解裡的數字沒有任何東西在檢查它**。另外抓到三句**被自己上一行資料否定
  的無條件斷言**（「在兩種規則下都看得見」「月 5 的變化仍然看得見」「這個 note 沒有多給什麼」），
  全部改成計算後才印的分支；以及兩個混進註解的中文字（輸出必須 ASCII，註解也不該有）。

  **一次量測否定前提，處理方式：** `the-second-time-was-faster-and-that-is-all` 寫的時候預期
  總損害會**上升**，量出來是**完全持平**（28 → 28）。持平其實是更利落的結果——兩個效應剛好
  抵消、沒有量級可以爭論——所以**改標題不改資料**，並補上相等時的分支，讓「沒印出來」不會變成
  靜默。

  **2026-08-17（第 76-78 輪）— 看得見的那一個，代替了它所屬的整體。** 語料庫 **411 → 426**，
  測試 **2311 / 61 files**，軸仍是 16，監控 426 支乾淨、追 27 個構件。主題連續第十六、十七、
  十八次從前幾輪讀出來：73-75 走「制度產物被當成它所代表的東西」；76 走它的來源——**為什麼
  某一個實例會被當成整體**；77 從 76 讀出「被記住的那一個之所以被記住，是因為它留下了痕跡」，
  整輪量**寫下來的契約 vs 觀察得到的行為**；78 則不是從語意讀出來的，是從**語料庫自己的構件
  覆蓋量出來的**——在第 78 輪之前的 **431 支** `.eml` 上用 `grep -F` 量，`^-`／`^*`／`^/`
  **各只出現在 1 支**、`continue` **13 支**、`abs()` **8 支**、`float()` **14 支**，所以第 78 輪
  整輪打這些薄的地方。

  **第 76 輪（顯著的單一實例）：** 大家都記得的那次事故佔全年停機 **28%**，沒人叫得出名字的
  那一類佔 **43%**，而預防投入是 **90% vs 6%**；沒有人被全員 call 的事故合計佔停機 **71%**、
  拿到 **9%** 的投入。一條規則因為 **1 個**反例被撤掉，它在 50 案裡對 **47** 次、接手的新規則
  對 **36** 次，而**連「完全不設規則」都對 37 次**——新規則比沒有規則還差 1，卻仍被偏好，因為
  它的錯是看不見的那種（舊規則 3 個可見錯、0 個不可見；新規則 0 個可見、14 個不可見）。修好
  一個真缺陷，**4 個** caller 壞掉、**2 個**受惠，而壞掉的四個沒有一個做了程式碼曾經拒絕過的
  事。同一組 12 個月的數字，最後一步是 **-5**、整年是 **+19**，用「最新一步」讀方向的正確率
  **50%**（5 對 5 錯）、用兩個月窗口是 **88%**。README 範例那個 shape 佔流量 **22%**、佔測試
  **77%**、佔缺陷 **5%**，其餘四個 shape 合計流量 78%、測試 22%、缺陷 **94%**。

  **第 77 輪（寫下來的 vs 觀察得到的）：** 四個公告管道各自真實、各自有訂閱者，合計觸及 11 個
  caller 中的 **4** 個，**7 個一個都沒收到**；而把警告放進 response header 觸及 **11/11**，因為
  它的受眾是由「呼叫」這個動作本身定義的。錯誤訊息改寫得更好，**7 個** consumer 裡 **4 個**壞掉，
  而壞的全部是比對文字的那 4 個、比對 code 欄位的 3 個**一個都沒壞**——穩定欄位完全照承諾穩定，
  只是文字才是出現在 log、截圖和工單裡的那一個。有文件的 endpoint 佔流量 **5%**、無文件的佔
  **94%**，而「versioned + validated + rate-limited」只覆蓋那 5%。一個佔營收 **4%** 的客戶佔了
  已出貨工作的 **81%**，而**沒填過任何 request 的 3 個帳號合計佔營收 35%**、拿到 0 項。文件寫著
  順序未定義，**9 個** caller 有 **7 個**依賴它——活下來的 2 個一個先排序、一個從不讀位置。

  **第 78 輪（語言自己的薄構件）：** `for i in [2:5]` 走 **4** 個值、`s[2:5]` 給 **3** 個字元
  ——同一個 `[a:b]` 記法，for-range 含右端、字串切片不含；超出結尾的切片回空字串而不是錯誤。
  `^+`／`^-`／`^*` 保留型別、`^/` 一律變 float：兩個 100 halving 迴圈步數都是 4，**4/4 步印出來
  不一樣、只有 2/4 步真的是不同的數**。迴圈頂端的 guard clause 跳過的不是那筆記錄，是**它下面
  的每一行**——`continue` 版本看到 7 筆、只數到 4 筆，而放在 guard 下面的 running maximum 在
  「最大值那筆剛好無效」的輸入上從 95 掉到 40。`abs()` 讓兩個模型的絕對誤差都是 **18**，
  signed error 卻是 **0 vs 18**；用「超額 2／缺貨 5」計價時**有偏誤的那個反而便宜**（36 vs 63）。
  `float("3.10") == float("3.1")` 為真而文字不同：8 筆讀數 **7 個相異文字、3 個相異數值**，
  小數位數這個關於儀器的宣稱只寫在 parse 不保留的那個地方。

  **今天抓到自己的一個真缺陷（不是散文，是程式）：** `contains()` 寫成 `hay[i:i+n-1]`，對所有
  輸入都回 0，而**出來的統計看起來完全合理**（「4 個 consumer 壞掉」本來就是預期值）。是直接
  去探測切片語義才發現的，順帶量到上面那條 range／slice 不對稱。另外一次**量錯了量測本身**：
  數構件覆蓋率時 grep 把 `^+` 的 `^` 當成行首錨點，改用 `grep -F` 重量。散文標題數字仍有三次
  （「right 31 times」實際 36、「last month is down」實際 up、「reached 3 of the 11」實際 4），
  第 78 輪第 2 案的標頭還寫了「stops comparing the way it did」——而程式自己下一段就印出
  `d == 3` 仍為真。**處理方式是把 README 產生器改成不可能打字**：標題、hook、說明全部取自程式
  自己的檔頭註解，每個輸出區塊都是跑出來抓的，結尾段落取自程式最後印的那幾行。README 現在無法
  聲稱一個程式沒有印出來的數字。

  **兩件驗證紀律的落實：** (1) 產生 README 前先 `pnpm build:cli` 重建 bundle，並與 source 執行
  結果**逐 byte 比對確認相同**才拿來用——08-14 那次「dist 是一個月前的建置」的教訓，這次是先
  演練再使用。(2) 監控說 426 支而 `examples/` 有 433 個目錄，**先把差額查清楚**才接受「無漂移」：
  監控排除 `phase*` 前綴目錄（6 個目錄、10 支 .eml），426 = 436 − 10，且與 426 份 README 一對一。

  **2026-08-18（第 79-81 輪）— 訊號沒問題，能做到什麼由它下游決定。** 語料庫 **426 → 441**，
  測試 **2356 / 61 files**，軸仍是 16，監控 441 支乾淨、27 個構件。主題連續第十九、二十、
  二十一次從前幾輪讀出來：76-77 的十個結尾句幾乎是同一個句型（「測試套件是用手上有的輸入寫的」
  「roadmap 是用寫下來的東西建的」「caller 是照他們看得到在動的東西建的」），全部指向**可得性
  決定內容**；79 走那條鏈的下一段——**把東西弄成看得見是標準解法，這輪量那個解法實際買到什麼**；
  80 走再下一段——**收到訊號的人用的是他手上有的動作，不是被要求的那個**；81 走這些局部正確的
  回應**累積之後**的樣子。

  **第 79 輪（弄成看得見之後）：** 一天 **21250** 則警告，全部正確、全部指名被棄用的呼叫，而能
  據以改一行程式的 caller 是 **2 個**（8 個裡 3 個被 log level 擋掉、1 個沒人讀那條 stream、
  2 個不擁有那個呼叫點）；按流量算，**14750/21250** 的呼叫來自警告到不了的 caller。加在「以前
  壞過的路徑」上的 log 讓 log 說失敗都在那些路徑：稽核找到 **59** 次、log 只有 **13** 次，
  **77% 看不見**，而 log 的最糟路徑是 auth refresh（6）、稽核的最糟是 bulk import（**22**）。
  同一週靜音的兩個 alert 之後的歷史**完全相同（都是 0）**，底下的條件差 **31** 次；靜音那個噪音
  alert 是對的決定（375 則警告對 30 件真事，**8%**），它同時移除了唯一在數這件事的儀器。錯誤率
  **3.0%** 精確且是預算判斷的正確工具，能解決的客訴是 **0 件**（1% 取樣要 100 件客訴才期望命中
  1 件）；月比 3.0% → 2.0% 是真的改善，**沒有點名剩下 760 件裡的任何一件**。平台團隊看得見
  **8/8** 個 finding、能改 **1/8**；服務擁有者能改 **6/8**、被告知 **3/8**；兩邊都握有其中一種
  權限的只有 **3/8**，其餘 5 個拆成「送個訊息就好」3 個跟「根本沒有 owner」2 個——兩個不同的修法。

  **第 80 輪（手上有的那個動作）：** 唯一能調的數字是 timeout，調高把 **195** 次/分的失敗變成成功
  （那些呼叫本來就會成功，只是被切斷），慢呼叫數 **220 → 220 完全沒動**，而佔住的連線時間
  **+54%**。重啟 4 分鐘、修要 120 小時：在搜尋的 **520 週**內重啟**從來沒有超過**修的成本
  （5 年 2080 分鐘 vs 7200 分鐘），所以「只看時間就別修」在五年內是對的——時間看不見的那一欄是
  五年 **20800** 個被丟掉的 in-flight 請求，而一個 8 小時的 drain 把那欄歸零、比全修便宜 **15 倍**。
  邊界的 handler 是對的位置，六個成因到那裡變成一種型別：一種回應對 2 個成因正確、對 2 個**更糟**、
  對 2 個沒差，需要的相異回應有 **5 種**、邊界能選的有 **1 種**。人工複查的覆蓋率是產能除以量：
  同一個人、同樣的工作，覆蓋率 **20% → 6%**，每月抓到的數字**平坦地維持在 30 上下**而漏出去的
  **120 → 420**。清洗器修掉了 **1760/1857** 的品項，代價是消費端自己的 guard 每天看到的量從
  **1555 掉到 55**（**96% 的證據沒了**），而下一個沒人見過的形狀正好是只有那個 guard 擋得住的。

  **第 81 輪（累積之後）：** 12 個 flag 各自為了一次真事故而加、各自單獨測過，允許的組合是
  **4096**，而 production 實際跑的設定是 **23** 個、測過 **7** 個——要處理的是 **16**，不是 4096
  （「4096」是個沒人部署的上界）。11 個 workaround 裡條件已經消失的有 **7** 個，其中拿得出證據的
  只有 **4** 個，**3 個是死的但沒人能證明**——謹慎在保護的正是這 3 個什麼都不是的東西；讓每個
  workaround 記錄自己的條件有沒有觸發，可證明可移除的就從 4 變成 **7**，一個都還沒刪。14 步的
  runbook 跑完 **35 分鐘**，四種事故各自真正適用的是 **7-11 分鐘**，最糟一次 **28/35 分鐘**花在
  不可能適用的步驟上，而第一行問一句「這是哪種事故」平均省 **26 分鐘**、一步都不用刪。四個團隊
  各自把自己那條線算對、各選最便宜的，總額 **68**；按總成本選是 **62**，省下的 6 由自己那條線從
  5 漲到 14 的那個團隊付。越舊的 workaround 條件越可能已經消失（三年以上 **5/6**）、越沒人看
  （三年以上一年 **0 次**複查，一年內 **9 次**）；同樣讀 5 筆，由新到舊找到 **1** 個可移除，
  由舊到新找到 **4** 個。

  **今天沒有出現散文標題數字的舊毛病**——README 產生器 08-17 改成從程式自己抓之後，那個欄位不
  存在了。抓到的是四個比較小的東西，全部在寫的當下就改掉：一個手打的 `str(9000)`（改成從資料
  算最慢的桶）、一個只是把 n 重數一遍的假計算、兩個整數截斷造成的假象（人工複查案每天算會被
  `int()` 壓成 0，改成按月算；控制組的覆蓋率沒有 clamp 而印出 **133%**，改成 capacity 與 volume
  取小）。另外一個邊界讀錯：3.0% 對上 3.0% 的預算印成「還有餘裕」，補了相等分支。

  **兩件驗證紀律照舊先演練再使用：** 產 README 前重建 CLI bundle 並與直接跑 source 的輸出逐
  byte 比對相同；監控 441、`.eml` 451、README 441——451 = 441 + 10 支 `phase*` fixture，
  三個數字互相對得上才接受「無漂移」。

  **2026-08-19（第 82-84 輪）— 回應本身是下一輪的輸入。** 語料庫 **441 → 456**，測試
  **2401 / 61 files**，軸仍是 16，監控 456 支乾淨、27 個構件。**今天的主題不是從語意讀出來的，
  是從語料庫自己的形狀量出來的**，而且量到的第一件事是關於我自己的：76-81 那六輪、十五個案例的
  結尾句幾乎是同一個句型（「X 是對的，而決定結果的是另一個量」）。用正則量整個語料庫：最近寫的
  60 個案例有 **35%** 在結尾明說「這個決定是對的」，較舊的 381 個只有 **7%**——**五倍的集中**。
  再量程式形狀：**63% 的案例在比較兩個具名的東西**，而「輸出回饋成輸入」只有 **4/441（0%）**、
  `while` 迴圈 6%、跨期間的時間序列 2%。語料庫幾乎全是靜態的兩欄比較，**動態幾乎不存在**。
  第 82-84 輪就補這個缺口，三輪全部是要跑很多輪才看得出結論的程式（黃金 trace 因此暴增，
  第 82 輪第 1 案 8274 行）。

  **第 82 輪（單方回饋：你的回應是你的下一個輸入）：** 一開始想寫「追著佇列跑的 scaler 會震盪」，
  **量出來否定了這個前提**——比例配置加一輪延遲會收斂，而且在每組參數上都贏過固定分配。真正的
  結構是：**固定 50/50 才是發散的那個**（飽和時 1890 vs 1134 backlog-interval，靜態那條佇列
  62→84 無界成長），而延遲的代價是一個隨餘裕縮小而變化的百分比——**8%（餘裕 4）、26%（餘裕 1）、
  11%（餘裕 0）**，最糟的不是最緊的那一組，因為餘裕 0 時兩種政策都已飽和。快取排名把自己的輸入
  之一算進去：從錯的起點出發**兩輪就鎖死在 {a,b,e,f}**，真正的前四是 {a,b,c,d}，c 和 d 永遠回不
  來——**拿掉 boost 後四組配對全部反轉**。失敗後多等一天：等待被存成 batch size，而 batch size
  正是下一次失敗的來源，30 天後間隔跑到上限 30 天、29 次故障，反過來「失敗後更快部署」是 4 次，
  兩者出貨量同為 180。估算被墊高後墊高的部分被吃掉：花掉 **177 天**做 **137 天**的事（+29%），
  而**從不墊高的政策 142 天完成、報表上多 3 次逾期**。過載時先砍「沒人在等的」：第 2 個 interval
  就開始把**所有使用者流量**砍掉（痛苦 115 vs 6），因為砍掉 16 單位的維護流量會還回 54。

  **第 83 輪（雙方回饋：兩邊各自對對方調整）：** 上下游各自加佇列，各自的丟棄率都真的歸零
  （21 → 0），而上游報 23、下游報 24、**一個工作單位實際等 47**，較大的那份報告是真相的 **51%**。
  兩邊各自為對方的破壞性變更寫 shim：8 個不相容產生 **13 個 shim**，5 個被雙邊各修一次，**8 個
  shim 什麼都沒防**，重複讓每次改動多花 15 小時。審查變嚴、作者變鬆：**量出來的結果又否定了我的
  前提**——這對搭檔收斂到的正是那個預算下的**最佳分割**（作者 0／審查 9，逃逸 2，十種分法裡最好
  的）。真正的發現因此是**路徑而不是終點**：第一天就選對的話 12 輪逃逸 24 個，實際逃逸 **53** 個，
  多出的 **29 個（+120%）**就是用互相調整去找到那個分割的代價，而兩邊都只知道自己的生產力。
  SLO 由「客戶要求」決定而客戶照著公布的數字回答：**6/6 的回答等於公布值**，而各自的自有承諾
  推出來的真實需求是 4 個要更少、1 個剛好、**1 個要更多**——那一個正是目標沒涵蓋的。兩隊的日期
  各自從對方的日期推導：一致到週，而**任一方多出 3 週工作時兩個日期一動都不動**。

  **第 84 輪（從迴圈裡的一輪去讀它）：** 六個月的改善曲線，一個「回到靜止點」的迴圈**逐月完全
  吻合（最大差 0）**，兩種讀法要到第 12 個月才差 18%。A/B 測試乾淨、隨機、樣本大，而**42% 的
  lift 是控制組被拖慢**，全量上線只拿到報告值的 47%，**十倍樣本一點都不會改善這個偏誤**。放大
  與衰減的系統對同一衝擊**第一個 interval 完全相同（都是 140）**，第二個 interval 的步伐是
  +10 與 -10；兩種猜錯的代價不對稱（閒置 33 vs 短缺 147）。兩週後的複盤讀的是總和的第一項：
  最後一筆成本在第 40 週落地，**5 個變更在第 2 週全部好看，其中 1 個到第 26 週是負的**。測試數
  與建置時間十二週一起動，觀察到的斜率是 **2** 而真實成本是 **3**——負回饋**壓低**了表觀效應，
  外生推一把量到的是 3。

  **今天自己的缺陷都在「印出來的斷言被自己上一行否定」這一類，全部在寫的當下改掉：** 一句
  「延遲代價隨餘裕下降而變大」被 8%/26%/11% 三個數字否定（改成計算出最糟的那一組並解釋為什麼
  不是最緊的）、一句「每個 batch 都比上一個大」被 12/3/9/6/6/6 否定、一句「輸掉的差距正好是
  boost」不精確（改成印出拿掉 boost 後四組全部反轉）。另外三個是**空的可觀測量**：負載卸除案
  第一版的卸除迴圈**一次都沒觸發**（`28 > 28` 為偽），兩種政策因此「一致」——那是兩邊都什麼都
  沒做；緩衝案第一版兩個緩衝其實是同一個機制，三種設定輸出完全相同；迴圈-settling 案的參數讓
  預測與觀測差到 10，而 `worst_gap <= 2` 的守衛**正確地拒絕印出那句假話**。還有兩個計數撞號
  （29 次部署 / 29 次故障其實是 28 個失敗部署 + 29 次故障）跟一個初始條件假象（斜率 12 是還沒
  平衡的第一週造成的，改掉之後是 2）。

  **驗證紀律照舊：** 產 README 前重建 bundle 並逐 byte 比對；三個計數對帳（`.eml` **466** =
  語料 **456** + `phase*` **10**，README **456**）才接受「無漂移」。

  **2026-08-20（第 85-87 輪）— 岑衡回來了，語料庫這三輪直接餵回稽核。** 語料庫 **456 → 471**，
  測試 **2446 / 61 files**，軸仍是 16，監控 471 支乾淨。Neo 說 GPT 額度已回復，所以第一件事是
  貼 `EMLP-RELAY-0019` 把暫停六天發生的事完整交代給岑衡，並且**先量再說**：
  `git diff --stat f77a43f HEAD -- packages/` 無輸出，**產品碼真的一行沒動**；同期全樹 317 檔、
  333928 行新增全部是語料庫、文件與監控 ledger。22 個 finding、4 個 CRITICAL（含檔案行號）、
  兩條 `OPEN` 的協定補丁，全部停在他離開時的位置。

  **主題因此不是從語意讀的，是接著我丟給他的那個問題選的**：他的 22 個 finding 是在一個
  **幾乎全是靜態程式**的語料庫上找到的（08-19 量到「輸出回饋成輸入」只有 4/441），而動態程式
  可能打開一批照不到的失效模式。第 85 輪就去打那個面。

  **第 85 輪（跨迭代的累積：逐步檢查通過，整段跑下來不通過）：** 探測到一條**真正的 EML-P 語言
  事實**——十個 `0.1` 用迴圈 `+` 累加得到 **0.9999999999999999**，`sum()` 得到 **1.0**，兩者
  **不相等**；逐前綴比對顯示前 5 個前綴兩者一致、**第 6 個開始分歧**，而 `== 1.0` 的測試因此由
  「用哪個累加器」決定成敗，差額是 1.1102230246251565e-16。以步長當收斂條件的求解器在容差 1000 ppm
  下停在距離答案 **8732 ppm** 的地方（**8 倍**），而把「剩餘距離 ≈ 步長 ×(k-1)」放回停止條件，
  多跑 21 輪就落在 959 ppm 內——預測 8730 對上實測 8732，兩邊獨立算出來差 2。逐行四捨五入與整筆
  四捨五入差 **1 分錢**，而**每一行的誤差都不到一分錢**。16 位元序號用 `>` 比大小，在第 **9363**
  則訊息翻轉（65534 → 5），而 100／1000／5000 三種測試長度**全部通過**。三個 worker 的部分和按
  完成順序相加：**6 種順序裡只有 2 種保住那筆手續費**，`sum()` 兩種順序都對。
  **另外量到 `a < b < c` 串接比較在 EML 是 `E_PARSE`，不支援。**

  **第 86 輪（條件的形狀）：** 把守衛子句挪到後面「比較好讀」，5 個輸入裡 **2 個直接除以零**，
  其餘 3 個兩種順序完全一致——所以它過了 review 也過了測試。用「值是空的」當「沒給」的判準，
  8 個設定裡 **2 個**被誤讀（`retries` 要 0 拿到 3、`rate_limit` 要 0 拿到 100），另外 2 個同樣
  被誤讀但預設值也是 0，**永遠不會出現在任何 bug report**。手動分配否定號把 `not (a and b)` 寫成
  `(not a) and (not b)`：四種組合**只有 2 種一致**，10 筆記錄裡放行了 **5 筆**該被跳過的。
  `admin or owner` 的授權條件在實際流量裡 **owner 子句一次都不是決定者**，拿掉它答案完全不變——
  哪個子句是死的是**打電話進來的人**決定的。事故後加的驗證子句擋下了那個 payload，同時**誤擋 4 筆
  合法流量（40%）**，而舊規則本來就抓到了全部有該標記的畸形 payload，**新子句只加了拒絕、沒加保護**。

  **第 87 輪（同一個值跨越邊界）：** id 走過一個用 double 的 hop：6 個裡 **3 個變了**，
  9007199254740992 與 ...993 **撞成同一個值**——而且**不是我建模的**，是用 `int(float(v))` 讓機器
  自己算（第一版我手寫的捨入模型間距差 2 倍、又用了 round-half-up 而非 ties-to-even，改成量測後
  整類錯誤消失）。單位寫在欄位名裡：10 組配對有 6 組跨單位，而**那 6 組拿原始值比較全部得到相反的
  答案**。五個 hop 各自誠實記錄自己的解析度，鏈的解析度是**其中最小的那個**，1234567 微秒走完只剩
  1000000。時區在某一 hop 被丟掉：7 筆有 **5 筆意義改變**，而**全部 7 筆通過範圍檢查**——不是超出
  範圍，是範圍內的錯誤時刻。schema 宣告成陣列是對的，而 7 個 consumer 有 **5 個只讀第一個元素**，
  48 萬則訊息**沒有一則長度不是 1**；空陣列會大聲失敗、兩個元素會安靜給錯答案，跟 review 時的排序相反。

  **今天自己的缺陷：** 一個 111 MB 的黃金 trace（`the-test-was-shorter-than-the-wrap` 第一版跑
  20000 次迭代，是全語料庫次大檔的 7 倍）——改成**把翻轉點用算的、再在邊界驗證**，變成 **68 KB**，
  結論不變而方法更好。一個手寫的 IEEE 捨入模型有兩個錯（間距差 2 倍、捨入規則用錯），改成直接量測。
  一個**恆真的 control**（`if r[1] == r[1]`）。一個最大絕對值追蹤寫成 `err < 0 - worst` 對負值永遠
  不觸發。加上三個散文小錯：「the loop is exact」被自己表格的 0.30000000000000004 否定、
  「8921 到 8928 較小」印的是迴圈結束後的值而不是失敗當下的值、一個 `str(3)` 手打的預設值。

  **驗證紀律照舊：** 產 README 前重建 bundle 並逐 byte 比對；`.eml` **481** = 語料 **471** +
  `phase*` **10**，README **471**，三方對帳後才接受「無漂移」。

  **2026-08-21（第 88-90 輪）— 修正戰役開工，語料庫主題全部由量測選出。** 語料庫 **471 → 486**，
  測試 **2491 / 61 files**，軸仍是 16，監控 486 支乾淨。

  **今天最大的事不是語料庫，是 EMLP-AUDIT-001／002 的 patch 交回。** 岑衡在 `EMLP-RELAY-0025`
  裁定 binding-order 見證**併入 001**（同一條資料流、同一個 root cause，另立會讓同一個修補義務
  被拆成兩件、其中一件可以單獨關閉），並給了六條驗收義務；002 維持獨立 root cause。她還做了我
  沒要求的事：**把我的測試放進語言 HEAD 獨立重跑一遍**，不是只讀我提交的紅燈輸出，跑完把暫存測試
  刪掉、不留產品修改。GitHub 權限也自己查證了（同一組 owner 身分、admin/push），而且**沒有為了
  試權限造一個空 commit**。

  **001 的修法是 one-at-a-time**：每個 numeric 變數輪流變動、其餘釘在 BASELINE，變數數線性而非
  指數，殺掉每一個「候選忽略變數 v」的 witness。依 name 排序**只是**讓生成不依賴呼叫端順序——
  **排序不是修正**，drill 1 就是為了證明這件事（保留排序、只恢復 first-only，001 照樣紅）。
  界限照 §4.5 寫出來：`MAX_SETS` 是**軟**上限，變數多時縮的是每變數取值數、**不是變數集合**，
  所以寧可超出上限也不留下未變動的變數。**002 是結果分類**：恰好一邊失敗＝分歧，回 false 並指名
  哪一邊；兩邊都失敗＝輸入不可用，計數並寫進 detail。「生成更多輸入」不會修好 002，這是它分開修
  的理由。三個 drill 各附實際紅燈與**機制觸發次數**，關鍵在第二半：**drill 1／2 讓 001 的 3/3 紅、
  002 的 0/4 紅；drill 3 讓 002 的 3/3 紅、001 的 0/6 紅**——兩個 root cause 各自獨立瞄準。

  **一件我查了而不是推論的事**：drill 2 裡 permutation 測試的失敗訊息是 `expected true to be false`，
  而**它的兩個斷言失敗時會印出一模一樣的訊息**，訊息本身分辨不出是哪一半紅的。直接對變異後的 build
  跑四種組合：`a=1/b=3` → **true（誤判）**、`b=3/a=1` → false、`a=1/b=1` → false、`b=1/a=3` → false。
  invariance 斷言確實是紅的那一半。**而這張表比原主張更尖**：第三列也是 a 先卻被抓到，所以誤判還
  需要多一個巧合——**LLM 必須把被忽略的變數綁成 validator 釘住的那個值**。那不是緩解：一個把 `b`
  折成常數的模型，正是會回報 `b = 3` 的那個模型。**巧合不獨立於缺陷，是缺陷造成的。**

  **三輪的主題全部由量測選出，不是從語意讀的。** 量整個語料庫的主題分布，找出真正薄的：
  「**只因為沒人試過才成立**」**0/471**、「**繼承自前人**」9/471、「**兩個永不相遇的群體**」3/471。

  **第 88 輪（沒人試過的路徑）：** 四年沒被碰到的上限——`0/12` 個月觸發，而它背後的拒絕路徑
  **5 步有 2 步指向已經不存在的東西**，而以目前成長率**還有 47 個月才會第一次跑到**。從沒跑過的
  rollback：5 個 migration **只有 2 個的 down step 全部還解析得到**，而 schema 回得去、**379000
  筆之後寫進來的資料回不去**。沒人傳過的參數：**9 個分支有 7 個（77%）只有原則上可達**，而 21 次
  review 提及裡 **19 次**是關於沒人傳的參數。只在最糟時才跑的 fallback：三年 4 次，**每一次失敗
  都在較糟的那一組**，而那組正是觸發條件本身篩選出來的。沒人查過的欄位：167 萬列裡 **64% 有值**，
  而有值的部分**橫跨兩種單位**，一份今天寫的報表會**靜默排除 35%**。

  **第 89 輪（繼承而來、你沒選的約束）：** 六年沒動的常數 4096——四個候選約束裡**同時「今天仍成立」
  且「等於這個常數」的有 0 個**，唯一對得上的那個已經消失。舊產品名：8 處出現、**現在指涉 3 種不同
  的東西**，而真正的代價不是名字過時，是**新團隊把同一個詞給了新的子系統**。schema 的六個約束裡
  **3 個編碼的規則已經廢止**，而每月 531 筆被擋的寫入裡 **425 筆（80%）**是被廢止的規則擋的。
  為一個已退休的整合塑形的介面：6 個特徵**3 個沒人用、3 個被別的 caller 撿走**，而撿走的那些
  「沒有人要求過那個形狀，他們只是照著看得到的東西寫」。繼承 oncall 的團隊：9 次 page 裡
  **6 次追到前一個團隊的決定、1 次追到函式庫作者**，而 **4 次來自沒有書面記錄的決定**。

  **第 90 輪（兩個永不相遇的群體）：** 團隊量到 44ms、使用者加權是 **282ms**。41 份回報背後
  **11780 個受影響使用者**，而**受影響最多的 segment 跟回報最多的 segment 不是同一個**，最吵的
  segment 每千人回報數是最安靜的 **16 倍**。凌晨兩點部署：**4 個區域只有 1 個真的在低谷**，其餘
  三個在自己尖峰的 71%／86%／81%，而總量之所以是最低點，是因為佔 55% 流量的那個區域在睡覺。
  11 個月後才出的修正：**受影響者只剩 79%**，而**被擋死的那群留存率 36%、只是難看的那群 91%**——
  修正抵達的是忍受下來的人。client 與 server 的儀表板都完整：**client 的 610 次失敗裡，server
  也記成失敗的只有 120 次（19%）**，還有 210 次兩邊都不算失敗。

  **今天自己的缺陷：** 一個把同一行印三次的迴圈（該取一次卻對每筆符合的資料都印）、一組不自洽的
  資料（某區域「在 02:00 的流量」超過它自己的「尖峰」，印出 108%）。都在寫的當下改掉。

  **2026-08-25（第 97-99 輪）。** 語料庫 **516 → 531**，測試 **2647 / 64 files**，軸仍是 16，
  監控 531 支乾淨。三個主題一樣走三種探針（窄詞／寬詞／複合詞掃全文），而且這次把探針寫成可重用
  的腳本而不是每天重推。窄詞給的候選裡，**寬詞再次推翻多數**：「安全補償」「回報者變成負責人」
  「per-user 公平」都在既有語料裡有真的鄰居。三個確認空的格子才動筆，其中兩個複合詞近零命中
  （`the-unit-was-in-the-name-not-in-the-value`、`abandoned-work-still-lands`）逐一開檔查證為
  誤判才計 0。

  **第 97 輪（兩個各自正確的防護，合起來互相致盲）0/516：** 限流器與斷路器串在一起——**跳閘原因
  的 99% 是限流器在做它該做的事**（本地產生的 429 與下游的 5xx 在斷路器輸入端無法區分），而讓
  斷路器只數 5xx 的控制組**多送 1500 個請求、跳閘 0 次**。清洗器跑在驗證器前面：7442 筆缺陷有
  **7400 筆在驗證前就被修掉**，驗證器只看到 56/10000，而**每天 200 筆被「修」成格式正確但語意錯的
  值直接進帳本**，驗證器對它們的拒絕數是 0；同樣兩條規則換個順序是 7442 對 42。自動修復加告警
  去重：708 次故障有 **704 次不會到任何人**，每週 1824 分鐘的降級服務**產生 0 個 page**，而唯一
  會叫人的故障是最罕見的那一個。緩衝區與背壓：緩衝把持續超載**藏 2 分鐘**，背壓在 t+3 才啟動，
  那時已經有 60 秒的存量要排；控制組（真正的突發）峰值 80% 容量、峰值延遲 48s 對 55s 門檻，自己
  排空。健康檢查與自動擴容：**在服務中的平均 71%、全體 81%，門檻 80%**——檢查移走的正是那些讀數
  說要擴容的機器，每台存活機器多扛 128 rps。

  **第 98 輪（懂它的那個人正是它保持複雜的原因）0/516：** 她不藏私、會結對、要過兩次重構時間、
  半夜兩點也回——而重構的預算看的是復原時間，復原時間是**帶著她量的**：**25 分鐘對 289 分鐘**，
  以量到的數字看 5 類事故 **0 類**超過 120 分鐘門檻，換成別人是 **5 類全超**。一種只有一個人會寫的
  設定格式：請求量長 **311%**，中位回覆從 1 小時到 3 小時，而自助表單的門檻是 24 小時——負載落在
  她的行事曆上，不是落在佇列裡，而只有佇列有門檻。一個人審了 **98%** 的變更，能讀那個模組的人
  從 3 掉到 1，而「送給最可能抓到東西的人」這條規則**每一次套用都是對的**。工具難用但只有一個
  熟手：**加權平均 59 秒、未加權 432 秒**，門檻 120 秒不觸發，而 **5 個操作員裡有 4 個每次都超過**。
  相容層八季維護 60 人日、只有 5 個 consumer 搬完——**那 60 天沒有一天花在搬遷上，都花在讓搬遷
  沒有必要**。

  **第 99 輪（介面沒變，底下的意思變了）0/516：** enum 加了三個值，型別、欄名、序列化全沒動——
  **5 個 consumer 有 3 個沒更新**，各自的 else 分支把新值分別當成 pending 重試、當成 settled 計入
  週報、或直接丟掉，**沒有一個報錯**。`occurred_at` 從事件時間改成接收時間：兩種定義**只在管線
  跟得上的時候一致**，8 小時裡只有 2 小時一致，同一個門檻下告警次數不同。訂單 id 從全域唯一改成
  租戶內唯一：**租戶對數從 3 長到 2415（805 倍），而租戶數只長 23 倍**，5 個查詢有 3 個現在是錯的
  而**一行 SQL 都沒改**。折扣率從 0.15 改成 15：3 個 consumer 行為變了、**0 個被編輯**，其中只有
  1 個會自己叫出來（顯示 1500%），而**測試夾具裡大於 1 的值有 0 個**。可為 null 的欄位被回填：
  5 個 null 分支全部變成不可達但仍然編譯、測試仍然通過（因為測試自己造 null 輸入），而回填**加權
  準確率 92% 代表引入了 23080 個錯值，且沒有任何東西標記是哪些**。

  **今天自己的缺陷：** 緩衝區那支的門檻設 1000 秒而延遲最大只有 60 秒，**背壓永遠不會啟動**，
  直接牴觸整支程式的敘述；同一支的控制組寫成 `capacity*100/capacity`（恆等於 100 的**同語反覆**），
  而且那個「突發」實際上會溢位。兩處百分比截斷成 0（42/7442、4/708），改用每萬／每千。一個
  `o[1]/o[1]*o[3]` 的空除法。

  **以及一個工具錯誤留下的殘骸，是測試套件抓到的不是我：** 有兩次 Write 回報 internal error，我改用
  heredoc 重寫並繼續。實際上那兩次**檔案有落地**，變成兩個沒有 trace、沒有 README 的孤兒目錄，主題
  與我完成的兩支重複。`tests/examples.test.ts` 的「每支 .eml 必須有對應的 committed trace artifact」
  直接紅了兩筆。我核對過內容確認是同主題的舊稿、且未進 git，移到 scratchpad 而不是刪除。
  **一個回報失敗的工具呼叫可能已經產生副作用**，而這裡是語料庫的完整性檢查替我發現的。

  **2026-08-23（第 94-96 輪）。** 語料庫 **501 → 516**，測試 **2602 / 64 files**，軸仍是 16，
  監控 516 支乾淨。三個主題全部由量測選出，而且**三個都經過三種獨立探針確認是 0**：先窄詞、
  再寬詞、再複合詞（全文而非只有檔頭）。寬詞那一輪**推翻了我自己七個 0 裡的五個**——
  「安全補償」實際有 3 支、「回報者變成負責人」7 支、「per-request 公平但 per-user 不公平」12 支，
  都是真的鄰居而不是誤判。剩下確實空的三格才拿來寫。

  **第 94 輪（決定的成本由沒有做決定的人付）0/501：** 每個 caller 自己設 retry——把所有人壓到
  2 次**省下 0 個請求**，因為第三次之後早就趨近於零；**第一次 retry 佔了成本的 99%**，而那正是
  沒有人會提議拿掉的那一次；**retry 設最多的（6 次）不是加載最多的（1 次 retry 的 search，
  +18000/日）**。producer 加一個必填欄位：自己 1 人日，consumer **17 人日（17:1）**，6 個裡 4 個
  嚴格解析器當場壞掉，而**最晚被發現的是量最小的那個（40000 筆/日，9 天）**；producer 自己的
  測試套件全綠，**它涵蓋的 consumer 數是 0**。升級被延八季：**「這一季出了什麼」這條規則 8/8 全
  延，而且每次都對**，因為升級在自己那一季的產出是 0；換成邊際規則（這季的價值 vs 多等一季會多
  出的天數）仍然**延對 6 次、只在第 7 次錯**——成本從 2 天長到 34 天（×17），而每季固定 2 天的
  版本總共只要 16 天。共用測試套件裡一個 40 秒的測試：**一年 460 工程師小時**，而 180 次 merge
  裡**只有 6% 有可能讓它失敗**；只在 ledger 路徑上跑可省 **93%** 而保護完全相同。log level 各隊
  自己設、平台付帳：**最吵的不是最忙的**（notifier 733333 行/千請求 vs 最忙的 auth），payments
  一家佔 3634 GB 的 70%，而**平台能改的 level 數是 0**。

  **第 95 輪（共識是因為反對的人離開了）0/501：** 設計評審的異議每季下降——**每提案降 89%，
  但改用「每出席者」校正後降 74%**，所以大部分是真的、有一部分不是；邀請名單從 22 長到 26，
  房間從 19 縮到 9，**曾經提過異議的 11 人裡有 9 人已經不在房裡**，而能分辨兩種解釋的那個量測
  （請一個已離開的人審一份現在的提案）**做過 0 次**。沉默記為同意：**59% 的「核准」來自窗口關閉
  而不是來自任何人**，改成需要明確過半則 **6 個提案 0 個通過**（現制 4 個），而回覆率完全跟著
  窗口走（21 天 100%、3 天 11%）。18 個月只有 2 次例外申請：**申請與否被那個 14 小時門檻完全解釋**
  （便宜的 6 件一件沒申請、貴的 2 件都申請且都獲准），**36 小時的順從成本一筆紀錄都沒有**；把
  申請成本掃過 2→40 小時，規則一個字沒改而證據從 7 件變成 0 件。棄用 API 的抱怨歸零：**抱怨
  −100%，流量只 −22%**，8 個抱怨過的人裡 **5 個還在打**，佔剩餘流量的 93%，而 71 件抱怨裡 29%
  是以 wontfix 結案的。合規衝到 100%：**0 個 repo 改過程式碼**，5 個被封存的裡 3 個仍在線上服務
  30300 請求/日；同一個標準有**三個都站得住的數字**（依流量 99%、依 in-scope 計數 100%、依全部
  計數 50%）。

  **第 96 輪（加了容量，需求長出來把它填滿）0/501：** worker pool 加倍——**二月其實超時 6 分鐘，
  但在「小時」這個單位下它跟準時完成印出同一個數字**（所以全程改用分鐘）；三月多出 267 分鐘餘裕，
  六月歸零，而**每個 worker 的負載回到加倍前的 1% 以內**。磁碟加倍：**保留天數 +200%、到達量只
  +7%**；把政策固定則 38 TB（80 TB 的一半不到），把到達量固定則 108 TB；而**兩次放寬的原因相反**
  ——第一次是在 46% 滿時（真的有空間），第二次是在 93% 滿時（那是第一次的後果到期）；30 天以上的
  資料被查詢的比例是 **0/1000**。CI runner ×3：**工作量只多 5%，但跑了多 200% 次**，使用率
  92% → 92%，佇列 11 → 12 分鐘，而每次 push 的 commit 數 4.0 → 1.4 **且不會因為佇列回來而回升**。
  rate limit 60 → 300：**99% 的回應現在是相同的**，而「有可能回傳新東西的 poll 數」完全沒動，
  因為那是資料變動頻率的性質；唯一沒跟著漲的是那個照自己排程跑的 caller。記憶體上限 4 → 16 GB：
  **kill 回來了**（新工作量對新上限每月 9 次），嘗試數 +17% 而要求的記憶體 +171%，**當初要修的
  那三份報表只佔增量的 3%，比例 28:1**。

  **今天自己的缺陷（12 個，全部在寫的當下改掉）：** 三次**檔頭宣稱被自己的程式推翻**——延後升級
  說「每次都對」而程式算出 8 次裡只有 3 次、例外申請說「18 個月零申請」而程式算出 2 件、
  記憶體上限說「三份報表跑不動」而程式算出**被 kill 的是 0 件**（那三次嘗試根本沒進資料）。
  一個**我自己編的排隊模型**在我聲稱等 11 分鐘的那個月回傳 0——改成把佇列當**觀測資料**放進去，
  不再模型化。一組資料讓 commit 數**下降 28%**，跟前提相反。磁碟那支印著「因為有空間」而同一行
  印出 **92% 滿**（我抓的是變更後的狀態）。`o[4 - 1]` 索引到 bytes-per-line 而不是請求數。
  **三次截斷吃掉形狀**（auth 印成 0 行/請求、未稽核流量印成 0%、小時解析度蓋掉 6 分鐘超時）。
  兩個殘句、一組加起來 99% 的百分比。以及一個 **grep 假象**：`two-phase-commit-window` 因為路徑
  含 "phase" 被子字串過濾器排除，讓語料庫數少算 1——跟當年 `^+` 被當成行首錨點是同一類。

  **一個真正的儀器修正：** README 產生器一直是用 `packages/cli/dist/index.js`（打包產物）跑程式的，
  而那個 bundle 會對 source 過期。今天把**交叉檢查寫進產生器**：同一支程式同時用 bundle 和 source
  各跑一次，**兩者輸出不同就拒絕產生 README**。演練這個 guard 時連續兩次演練是**空的**——在 bundle
  尾端 append `console.log` 從來不會執行（CLI 先 exit），在最前面 prepend 又會破壞 ESM 載入（shebang
  必須在第一行）；只有**插在 shebang 之後**才真的改變 stdout，guard 才第一次紅：
  `INSTRUMENT MISMATCH ... dist=2103 bytes, src=2076 bytes`，差的 27 bytes 正好是注入的那一行。

  **修正戰役：** 001/002 昨天已落地 `7bc3100`，今天沒有新的產品變更。下一步是 `emitter.ts` 裡的
  003 / 004。

  **跨 session：** Elenchos（MSSP 線）問到身分。用「雙方 `ListAgents` 清單取差集」互相定名
  （session 不列出自己），確認我是 `ai-57`、他是 `ai-f1`。他請我跑 EveMissLab bridge probe 提供
  第二個觀測點：**seat 逐字元相同**（`agent://evemisslab/residence/claude-code`），其餘欄位也全部
  一致，所以那個字串是 **runtime 標籤不是身分**，不能用來定址人——已據此改掉 Plumb 的記錄措辭。

  **2026-08-22（第 91-93 輪）— 語料庫過 500。** 語料庫 **486 → 501**，測試 **2536 / 61 files**，
  軸仍是 16，監控 501 支乾淨。三個主題全部由量測選出，而且**三個都是量到 0 的格子**：
  「規則被不均勻執行」**0/486**、「稀缺資源給先開口的人」**0/486**、「正確修正的二階效應」**0/486**。

  **第 91 輪（同一條規則被不均勻執行）：** 分階段推行是對的排程——先警告、再軟失敗、最後阻擋，
  每隊拿到與工作量成比例的時間。而最後一步在**最難的地方最貴**，所以它在最該完成的地方沒完成：
  6 條 pipeline 有 4 條停在全面執行之下，**只警告的那兩條扛了 77 次違規中的 67 次（87%）**，
  而「政策套用率 100%」跟「違規到不了 production 的 pipeline 數」是兩個不同的數字。豁免流程本身
  沒問題，但**要開口才有**：8 隊裡 6 隊符合資格、5 隊知道流程存在、4 隊開口、3 隊拿到——**4 個
  符合資格的隊沒有豁免，其中 3 個根本不知道有這個流程**。同一條規則的 8 次違反：**嚴重度 5 的
  兩件共 5 小時，嚴重度 1 的一件 28 小時**，而**進到 review 的那組是嚴重度較低的那組**。同一份
  對每個 partner 都一樣的標準：**10% 營收以上的 partner 豁免率 100%，10% 以下 7%**，被擋下的
  四家合計營收 5.5%。慣例的遵循率隨與作者的距離衰減 **97% → 10%（4 跳）**，而**收益最大的模組
  在 3 跳、遵循率 20%**。

  **第 92 輪（稀缺的東西給了先開口的人）：** 會議室裡分預算——**在場的 4 隊用 57% 的運算、拿到
  87% 的預算**，3 個被少分的隊**全部缺席**。九個索引每一個都是「有人回報慢查詢」之後加的，
  每一次都修好了那個查詢：**淨效益是正的（+1.1M ms/天）**，因為兩個索引極為有效；而**9 個裡有
  6 個個別來看成本大於節省**，最差的五個都是「高寫入表上的低流量讀」——正是人會注意到的形狀。
  七件事被升級、七件都優先處理：**升級件平均價值 4.2，排隊件 7.8**，延遲成本 1131 value-days。
  速率上限三年沒動：**開口過的 caller 平均餘裕 966，沒開口的 18**，而**3 個沒開口的 caller 正在
  被限流**。共用環境先到先得：**價值 7 以上的工作只拿到 17% 的時數**，而排不了長程計畫的正是
  security 與 incident 這類工作。

  **第 93 輪（正確修正的二階效應）：** 加上驗證之後錯誤率從 0 跳到 4%——而**畸形資料的比率前後
  完全一樣（40/1000）**，改變的是門，不是資料；被移除的 **6010 筆髒資料在任何圖表上都看不到**，
  因為沒寫進去的列留不下任何東西可數。把最慢的階段加速 4 倍、整條快 38%，而**瓶頸移到一個少 4 個
  人開過、0 頁 runbook、19 個月沒人碰的階段**——因為一個階段被看的原因就是它慢。競態四年沒反轉過：
  **裕度從 300ms 降到 4ms，而「沒觀察到反轉」在每個版本都是同一個觀察**；五種抖動舊裕度吸收 5/5、
  新裕度吸收 0/5。修好 27 倍的 retry 風暴移除了 13% 的請求——**那些重複請求也是刷新**，低流量高
  重算成本的 key 因此掉出快取，而「請求量下降」「命中率下降」「p99 上升」三張圖全部正確且同因。
  去重省下 40% 儲存：**5 份副本每一份都有具名讀者**，而讀次最少的那份是「唯一讓其他四份可還原的
  東西」——恢復與對帳本來就設計成很少發生。

  **今天自己的缺陷：** 一個散文標題寫「整條快 12%」而程式算出 **38%**（移除該數字）、一個手打的
