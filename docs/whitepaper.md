...
subtitle: "從 MVP 轉譯器到 PHOSPHOR 可觀測執行層的技術白皮書"
...
## 副標題：從 MVP 轉譯器到 PHOSPHOR 可觀測執行層的技術白皮書
...
EML（Efficient Meta-Language）是一套面向人類與 AI Agent 共同編程場景的高密度語意附加程式語言／語言增益層。它的核心不是單純發明更多符號，也不是以符號壓縮替代所有傳統語法，而是將「語意附加」（Semantic Overlay）設計為可解析、可轉譯、可視覺化、可回放、可驗證的工程協議。
...
2. **EML 1.5 技術白皮書**：強調語意附加、資訊密度、冷熱數據分離、邏輯結晶化、Nova IME、Cogni-Editor、MSSP 等核心概念。
3. **EML 1.5 Ultimate 文件**：將 EML 推進到時間感知、自動修復、動態演化、BUG 免疫、十二種迴圈與動態編譯器的範式層級。
4. **EML-PLAN-2026 工程計畫書**：把上述概念重新落成六個月工程路線，並指出 PHOSPHOR 已可作為視覺化基礎設施。
...
範式層：冷熱分離、邏輯結晶化、十二種迴圈、時間迴圈、BUG 免疫
    ↓
工程層：Py⁺ 轉譯器、CLI、測試框架、Cogni-Editor、Nova IME、PHOSPHOR CTS
    ↓
...
> EML 2026 的第一目標不是證明所有終極能力，而是建立一個可被人類使用、可被 AI Agent 理解、可被標準語言執行、可被 PHOSPHOR 觀測的最小語意閉環。
...
早期文件曾使用 Efficient New Language 的命名，強調「高效新語言」。2026 工程版更適合使用 **Efficient Meta-Language**：高效元語言。

此處的「Meta」不是抽象裝飾，而是工程定位：
...
Cogni-Editor 的本質就是這種「雙態／三態投影」：同一段程式可以被機器看成壓縮符號，被人類看成可讀程式，被教學或審查場景看成自然語言解釋。
...
> 在本地環境中輸入一段 EML/Py⁺ 符號態程式碼，系統能將其解析為標準 AST，轉譯成可執行 Python，執行測試並在 Cogni-Editor/PHOSPHOR 介面中展示符號、展開碼、依賴與執行結果。
...
AST 是 EML 後續接上 PHOSPHOR、AI Agent、CTS、依賴圖與邏輯結晶化的共同基礎。
...
CLI 是 MVP 的工程脊椎。即使 Cogni-Editor 還不完整，CLI 也能讓 Agent、自動測試、CI/CD 與文件範例先跑起來。
...
這讓 PHOSPHOR 不只是展示執行結果，而能展示語義對應、符號依賴與執行 trace。
...
## 3. 系統架構：EML × PHOSPHOR × Cogni-Editor
...
│ Cogni-Editor, Nova IME, AI Convert Assistant  │
└───────────────────────┬──────────────────────┘
                        ↓
┌──────────────────────────────────────────────┐
│ EML Language Layer                            │
...
┌──────────────────────────────────────────────┐
│ Observability Layer                           │
│ PHOSPHOR VM/native trace, CTS, VMSnapshot     │
└───────────────────────┬──────────────────────┘
                        ↓
┌──────────────────────────────────────────────┐
...
PHOSPHOR 在 EML 中不是附屬 UI，而是「執行真相層」。EML 負責表達語義，PHOSPHOR 負責記錄發生過什麼。
...
### 3.3 Cogni-Editor 的角色

Cogni-Editor 是 EML 的人類適配層。沒有 Cogni-Editor，EML 很容易被誤解成「符號太多、難以輸入、難以閱讀」。有了 Cogni-Editor，EML 變成可切換視角的投影系統。
...
Nova IME 不是第一版的核心阻塞項。MVP 可以先用 ASCII 替代符號輸入。但 Nova IME 會決定 EML 的長期使用體驗。

第一版 Nova IME 可做成瀏覽器／編輯器浮動符號盤：
...
這點與 NOEMA/PHOSPHOR 的精神一致：Agent 可以執行，但執行要有 trace；Agent 可以修改，但修改要能 diff review；Agent 可以推理，但推理要能回到證據。
...
### Phase 1：Cogni-Editor 與 AI 雙向轉換
...
目標：形成 EML-PHOSPHOR 統一入口、文件、範例與 GitHub 發布。

交付物：

```text
EML.exe / eml desktop launcher
Cogni-Editor + Nova IME + PHOSPHOR minimal integration
...
5. 任何修改都要附測試。
6. PHOSPHOR trace 是執行真相層，不是裝飾 UI。
```
...
EML 不能只對 AI 友好。若人類在工具層完全痛苦，語言不會被使用。因此 Cogni-Editor 與 Nova IME 是產品化必要條件。
...
| 輸入困難 | Unicode 符號難打 | ASCII fallback + Nova IME |
...
| Agent 亂改 | 自動化導致破壞 | PHOSPHOR trace + diff review + policy |
...
## 14. 與 NOEMA / Noesis / PHOSPHOR 的長期關係

EML 不是孤立專案。它可以成為 NOEMA Agent OS 生態中的語言層：

```text
Noesis Studio：人類知識工作台 / Human Cockpit
NOEMA Agent OS：多模型與 Agent 協調層
PHOSPHOR：執行觀測與 trace 層
EML：Agent 可讀的語意附加程式語言層
```

四者的關係：

```text
人類在 Noesis / Cogni-Editor 中表達意圖
    ↓
EML 將部分意圖壓縮為可執行語義
    ↓
Agent 將 EML 轉成標準語言並執行
    ↓
PHOSPHOR 記錄實際執行 trace
    ↓
NOEMA 協調 Agent、模型、工具與文件
    ↓
人類審查 diff / trace / output
```

在這個結構中，EML 的價值不是取代 Markdown、Python 或 Agent，而是成為它們之間的高密度語義橋樑。
...
在這個命題下，語意附加不是表面符號，而是程式語義的壓縮接口；PHOSPHOR 不是外掛視覺化，而是執行真相層；Cogni-Editor 不是普通編輯器，而是人類與機器在不同視角間切換的投影介面；Nova IME 不是符號鍵盤，而是高密度語意輸入的摩擦降低器；Logic Crystallization、Temporal Loop 與 BUG Immunity 則是 EML 長期走向自適應程式語言的進化路線。
...
PHOSPHOR CTS 是必要輸出，不是額外功能。
Cogni-Editor 的第一目標是雙態視圖，不是完整 IDE。
```
...
4. Neo.K.《EML 工程計畫書：高效新語言（Efficient Meta-Language）漸進實現路線》, EML-PLAN-2026-v0.1, 2026-06-11.
5. EveMissLab / PHOSPHOR project notes, EML-EAI-2026 series.
