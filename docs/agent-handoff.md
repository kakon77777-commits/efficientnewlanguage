...
2. **Run `pnpm test` before and after any change.** 363 tests must stay green
...
`try/except`, `class`, user `import`, and `break`/`continue` remain explicitly
out of scope — deferred to a later round, not rejected.
