...
function cmdBugs(a: ParsedArgs): void {
  ...
  // Emit an EML eml-trace-v1 trace (always to memory; also to a file with --trace).
  ...
  if (typeof traceOut === 'string') console.error(`wrote eml-trace-v1 trace to ${traceOut} (${buffer.length} events)`);
...
/**
 * `eml trace` — produce an EML `eml-trace-v1` execution trace.
 *
...
function cmdTrace(a: ParsedArgs): void {
      ...
      if (ir.unsupported.length === 0 && ir.error === undefined) {
        ...
        for (const line of (py.stderr ?? '').split('\n')) {
          ...
          try {
            ...
            if (ev.proto === 'eml-trace-v1' && typeof ev.type === 'string') {
  ...
  console.error(
    `eml-trace-v1: ${events.length} events, ${anomalies} anomal${anomalies === 1 ? 'y' : 'ies'}` +
...
Usage:
  ...
  eml trace <file> [--out f] [--run] EML eml-trace-v1 execution trace (interp;
