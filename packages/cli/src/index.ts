...
import { createEmitter, memorySink, multiSink, findAnomalies, type PhosphorEvent } from '@eml/trace';
...
function cmdBugs(a: ParsedArgs): void {
  ...
  // Emit a PHOSPHOR-conformant trace (always to memory; also to a file with --trace).
...
/**
 * `eml trace` — produce a PHOSPHOR `phosphor-jsonl-v1` execution trace.
 *
    ...
    if (!python) {
      ...
      if (ir.unsupported.length === 0 && ir.error === undefined) {
        ...
        for (const line of (py.stderr ?? '').split('\n')) {
          ...
          try {
            const ev = JSON.parse(t) as PhosphorEvent;
...
Usage:
  ...
  eml cts <file> [--out f]           Emit PHOSPHOR-compatible CTS (JSON)
